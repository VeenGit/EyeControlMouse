...
while True:
    ...
    if landmarks_points:
        ...
        for id, landmark in enumerate(landmarks[474:478]):
            # these lines detect x and y axis from landmarks and then draw circles
            x = int(landmark.x * frame_w)
            y = int(landmark.y * frame_h)
            cv2.circle(frame, (x, y), 3, (0, 255, 0))
            if id == 1:
                # for moving cursor anywhere on the screen. Cursor moves only with movement of right eye
